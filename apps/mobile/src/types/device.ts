...
export function getTelemetryPurityPercent(
  ...
  if (!telemetry || telemetry.tdsReadingValid === false) {
    return null;
  }
  ...
  if (
    'waterQualityPercent' in telemetry &&
...
export function getTelemetryTdsPpm(
  telemetry: Pick<DeviceTelemetry, 'tdsPpm' | 'tdsReadingValid'> | null | undefined
) {
  if (!telemetry || telemetry.tdsReadingValid === false) {
    return null;
  }
...
export function getFreshTelemetrySnapshot(
  ...
  return {
    ...
    ph: typeof telemetry.ph === 'number' && Number.isFinite(telemetry.ph) ? telemetry.ph : undefined,
    turbidity:
