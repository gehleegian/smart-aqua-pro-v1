...
function buildQualityAlert(aquarium: Aquarium): AlertCandidate[] {
  ...
  const severity: AlertSeverity = hasPpm
    ? aquarium.tdsPpm! > 700
      ? 'critical'
      : 'warning'
...
function buildSystemAlerts(aquarium: Aquarium, shadow?: DeviceShadow, now = Date.now()): AlertCandidate[] {
  const alerts: AlertCandidate[] = [];
  const telemetryState = getDeviceTelemetryState(shadow?.telemetry, now);
  ...
  return aquariums.flatMap((aquarium) => {
    ...
    return [
      ...
      ...(sensorAquarium && snapshot?.hasFreshPurityTelemetry
        ...
        : []),
      ...buildSystemAlerts(aquarium, shadow, now),
    ];
  });
}

export async function readAlertHistory(storageKey: string) {
  try {
...
export async function writeAlertHistory(storageKey: string, alerts: AquariumAlert[]) {
...
export function buildDistribution(alerts: AquariumAlert[]) {
  const categories: { label: string; key: AlertCategory }[] = [
    { label: 'Temperature', key: 'temperature' },
    { label: 'Water Level', key: 'water_level' },
    { label: 'Water Purity (TDS)', key: 'water_quality' },
