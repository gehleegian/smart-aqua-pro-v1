...
export function createUserService({ auth, db }: UserServiceClients) {
  ...
  async function getCurrentUserProfile(uid: string): Promise<UserData | null> {
    const currentUser = auth.currentUser;
  ...
  async function getAllUsers(): Promise<UserData[]> {
    ...
    return snapshot.docs.map((snapshotDoc) => mapUserData(snapshotDoc.id, snapshotDoc.data()));
  }
